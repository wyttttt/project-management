...
//增
app.post('/addanswerList',async c=>{
    try{
        let data = JSON.parse(c.body)
        let sql = 'insert into answerList (aname,apage,aaddress,ayear,atype,afrequency,aprise) values ($1,$2,$3,$4,$5,$6,$7)'
	    let result = await pdb.query(sql,[data.aname,data.apage,data.aaddress,data.ayear,data.atype,data.afrequency,data.aprise])
	    if(result.rowCount<=0){
	        c.send('failed register',400)
	        return
	    }
...
//改
app.post('/updateanswerList',async c=>{
    try{
        let data = JSON.parse(c.body)
        let sql = 'update answerList set atype = ($1) where apage = ($2)'
	    let result = await pdb.query(sql,[data.atype,data.apage])
	    if(result.rowCount<=0){
	        c.send('failed register',400)
	        return
	    }
...
//删
app.post('/delanswerList',async c=>{
    try{
        let data = JSON.parse(c.body)
        let sql = 'delete from answerList where aname =($1)'
	    let result = await pdb.query(sql,[data.aname])
	    if(result.rowCount<=0){
	        c.send('failed register',400)
	        return
	    }
